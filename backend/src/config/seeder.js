...
const seed = async () => {
    try {
        ...
        const admin = await User.create({ name: 'Admin User', email: 'admin@nexus.ai', password: 'password123', role: 'admin', isActive: true });
        const demo = await User.create({
            name: 'John Doe', email: 'john@nexus.ai', password: 'password123', role: 'user', isActive: true,
            profile: { title: 'Senior Frontend Developer', bio: 'Passionate developer with 5 years experience.', skills: ['React', 'Node.js', 'TypeScript'], experience: 5, targetRole: 'Engineering Manager' },
            stats: { totalInterviews: 4, averageScore: 79, bestScore: 88, totalTime: 7200 }
        });

        const resume = await Resume.create({
            userId: demo._id, fileName: 'john_doe_resume.pdf', fileUrl: '', rawText: 'John Doe | john@example.com | San Francisco\nSkills: React TypeScript Node.js MongoDB AWS Docker\nExperience: TechCorp 2021-Present Senior Frontend Developer\nEducation: UC Berkeley BS CS 2019 GPA 3.8',
            parsedData: {
                name: 'John Doe', email: 'john@example.com', phone: '+1 555-0100', location: 'San Francisco, CA',
                ...
                experience: [
                    { company: 'TechCorp Inc', role: 'Senior Frontend Developer', duration: '2021 - Present', description: 'Led migration from AngularJS to React improving performance by 40%. Built real-time dashboard serving 50K daily users.', years: 3 },
                    { company: 'StartupXYZ', role: 'Frontend Developer', duration: '2019 - 2021', description: 'Developed e-commerce platform with React and Node.js. Implemented CI/CD reducing deployment time by 60%.', years: 2 }
                ],
                education: [{ institution: 'UC Berkeley', degree: 'B.S. Computer Science', year: '2019', gpa: '3.8' }],
                certifications: ['AWS Certified Developer', 'Google Cloud Professional'],
                projects: [{ name: 'Real-time Dashboard', description: 'WebSocket-powered analytics dashboard serving 50K concurrent users.', technologies: ['React', 'WebSockets', 'D3.js'] }],
